...
def _load_system_blocks() -> list[dict]:
    ...
    if _topology is not None:
        sections.append(
            ...
            "\n"
            "COPY-READY DRAFTS\n"
            "- When your answer contains an email, a WhatsApp/WeChat/LinkedIn "
            "message, or any other text the user is meant to send verbatim, put "
            "that text — and only that text — inside a fenced block tagged "
            "```email (or ```message for a chat message, ```reply for a reply "
            "to something they pasted).\n"
            "- The interface renders it as a labelled box with a Copy button, "
            "so the user can paste it straight into their mail client.\n"
            "- Keep your own commentary OUTSIDE the fence. Subject lines belong "
            "inside it, on the first line as 'Subject: ...'. Never wrap an "
            "explanation, a table, or a list of options in these fences — only "
            "text that is ready to send as-is.\n"
            "</interface_capabilities>"
        )
