...
def log_successful_login(email, role):
    """
    Silently logs successful logins to a local CSV file on the server.
    The user logging in will not see this process.
    """
    try:
        log_file = "master_login_records.csv"
        # Check if file exists so we can add headers if it's new
        file_exists = os.path.isfile(log_file)
        
        with open(log_file, "a", encoding="utf-8") as f:
            if not file_exists:
                f.write("Timestamp,Email,Role/Mode\n")
                
            timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
            f.write(f"{timestamp},{email},{role}\n")
    except Exception:
        pass # Fail silently so it never interrupts the user's login experience
        ...
        with col2:
            with st.form("login_form"):
                ...
                if submit_btn:
                    ...
                    if is_valid:
                        # Silently log the successful authentication on the backend
                        log_successful_login(email_input, message)
