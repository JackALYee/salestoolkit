...
1. **SMTP credential check** (`login.py`): user types `@streamax.com` email + password; `verify_streamax_credentials()` verifies by attempting an SMTP AUTH login against the backends in `_mail_servers()` — Coremail (`mail.streamax.com:465`, implicit SSL) then Microsoft/Outlook (`smtp.office365.com:587`, STARTTLS) — accepting the login if **either** authenticates, since Streamax mailboxes live on one system or the other. Per-server outcome is logged to stderr as `[LOGIN] …`. ⚠️ The Outlook path only works if that M365 tenant/mailbox still allows **SMTP AUTH (basic auth)** with no MFA/Conditional Access block; Microsoft disables SMTP AUTH by default on modern tenants (error `535 5.7.139` / "disabled for the Tenant", surfaced as `disabled=True`; note Microsoft reuses **5.7.139 for an ordinary wrong password too**, so the `disabled` test requires the literal disabled/blocked wording, never the status code alone) — which is why **Sign in with Microsoft** (below) exists. Set `MS_SMTP_AUTH=0` to drop the Microsoft SMTP probe once OAuth is live. Easter-egg shortcut accounts (`jerry_test`, `hekun_test`, etc.) bypass SMTP.

   **Non-ASCII passwords.** `smtplib.login()` hardcodes an **ASCII** encode of the credentials and raises `UnicodeEncodeError` before any network I/O — but RFC 4616 defines SASL PLAIN's password as **UTF-8**, so that limit is smtplib's, not the protocol's. A password containing e.g. a full-width `！` (U+FF01, what a Chinese IME emits for `!`) used to surface as *"Could not connect to the mail server: 'ascii' codec can't encode…"*, which is doubly wrong: nothing was ever sent. `_smtp_auth()` now routes non-ASCII passwords through `_auth_plain_utf8()`, which issues `AUTH PLAIN` by hand over `docmd()` with a UTF-8 payload — on a *fresh* connection, because smtplib tries CRAM-MD5 first and dies mid-SASL-exchange, leaving the session in a state where the next command reads as the challenge response. Failures are also classified positively now (`_looks_like_connection_error()`), so a client-side error can never again masquerade as a connection problem, and `_describe_non_ascii()` names the offending character in the user-facing message. Test: `python3 scripts/test_login_smtp_auth.py` (spins up a local SMTP server, no network).
...
## Identity flags on the HTML site (`server.py`)
...
⚠️ **Anything a side-car reads from `st.session_state` is blank here** — the stub's session_state is empty and stays empty. This has now bitten three times, each silently: `special_relationship` pinned to None, `usage_logger.log_query()` logging every turn as an anonymous `""`, and `_allowed_models()` never seeing a user's own Anthropic key (so BYO didn't unlock Claude on the HTML site even though the settings panel promises it does). **Whenever you call a shared module from `server.py`, check what it reads from session_state and pass it explicitly.** The fixed functions all take the value as an optional parameter defaulting to `None`, so the Streamlit path still resolves from session_state unchanged.
